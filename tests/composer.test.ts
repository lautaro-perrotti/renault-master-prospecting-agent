...
describe('email composition',()=>{
  ...
  it('validates template recipients and renders sender phone',()=>{const message=renderTransportIntroV1({companyName:'Empresa',recipientEmail:'compras@empresa.com',evidenceExcerpt:'entregas programadas en CABA',vehicle:'Renault Master',temperatureCapability:'refrigerada',coverage:['CABA','GBA'],senderName:'Operaciones',senderPhone:'11 1234 5678'});expect(message.to).toBe('compras@empresa.com');expect(message.body).toContain('entregas programadas en CABA');expect(message.body).toContain('11 1234 5678')});
});
