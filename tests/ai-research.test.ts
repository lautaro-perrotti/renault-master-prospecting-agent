...
describe('validación de evidencia de investigación',()=>{
  it('rechaza referencias desconocidas',()=>{const result:any={companyType:'DISTRIBUTOR',summary:'x',confidence:'MEDIUM',detectedSignals:[{evidenceId:'missing',signal:'x'}],relevance:0.5,recommendedAction:'REVIEW'};expect(()=>validateResearchResult(result,['e1'])).toThrow('AI_REFERENCES_UNKNOWN_EVIDENCE')});
