...
const campaignName='DEFAULT_REFRIGERATED_OUTREACH';
function profile(config:Config){try{return config.SERVICE_PROFILE_JSON?JSON.parse(config.SERVICE_PROFILE_JSON):{vehicle:'Renault Master refrigerada',temperatureCapability:'configurada según carga',baseLocation:config.BASE_LOCATION,coverage:config.SERVICE_COVERAGE.split(','),availability:'a coordinar',cargoTypes:['alimentos refrigerados y congelados'],certifications:[],documentation:[],contactPerson:'',phone:'',videoUrl:''}}catch{throw new Error('INVALID_SERVICE_PROFILE_JSON')}}
export async function draftHandler(db:Db,config:Config,job:any,_deps:JobDependencies){
  ...
  const message=existing??(await db.execute(sql`INSERT INTO messages(company_id,sequence_id,direction,to_email,subject,body,idempotency_key,sequence_step,outbound_state) VALUES(${companyId},${sequence.id},'OUTBOUND',${contact.email},${`Transporte refrigerado para ${company.name}`},${composeEmail(company.name,contact.email,evidence.map(x=>x.excerpt),profile(config),config).body},${`${sequence.id}:DAY_0`},'DAY_0','READY') ON CONFLICT(sequence_id,sequence_step) DO UPDATE SET body=messages.body RETURNING *`)).rows[0];
