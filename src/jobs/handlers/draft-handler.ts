...
export async function draftHandler(db:Db,config:Config,job:any,_deps:JobDependencies){
  ...
  const composed=composeEmail(company.name,contact.email,evidence.map(x=>x.excerpt),profile(config),config);const message=existing??(await db.execute(sql`INSERT INTO messages(company_id,sequence_id,direction,to_email,subject,body,idempotency_key,sequence_step,outbound_state) VALUES(${companyId},${sequence.id},'OUTBOUND',${composed.to},${composed.subject},${composed.body},${`${sequence.id}:DAY_0`},'DAY_0','READY') ON CONFLICT(sequence_id,sequence_step) DO UPDATE SET body=messages.body,subject=messages.subject,to_email=messages.to_email RETURNING *`)).rows[0];
