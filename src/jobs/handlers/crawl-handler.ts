...
export async function crawlHandler(db:Db,config:Config,job:any,deps:JobDependencies){
  ...
  for(const page of pages){
    ...
    for(const finding of extractContacts(page.text,page.url))if(finding.email)await db.execute(sql`INSERT INTO contacts(company_id,email,kind,role,source_url,verification_status,verified) VALUES(${companyId},${finding.email},${finding.kind},${finding.role},${finding.sourceUrl},${finding.verificationStatus},false) ON CONFLICT(company_id,email) DO UPDATE SET source_url=excluded.source_url,verification_status=excluded.verification_status,role=excluded.role`);
