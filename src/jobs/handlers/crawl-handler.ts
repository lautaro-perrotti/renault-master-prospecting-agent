...
export async function crawlHandler(db:Db,config:Config,job:any,deps:JobDependencies){
  ...
  for(const page of pages){
    ...
    const evidence=(await db.execute(sql`INSERT INTO evidence(company_id,source_type,source_url,title,excerpt,signal_type,confidence,observed_at) VALUES(${companyId},${page.sourceType},${page.url},${page.title||page.url},${page.text.slice(0,4000)},${signalFor(page)},${page.businessSignals?.length?85:55},${page.observedAt}) ON CONFLICT(company_id,source_url,excerpt) DO UPDATE SET observed_at=excluded.observed_at RETURNING id`)).rows[0] as any;
