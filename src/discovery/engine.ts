...
export class DiscoveryEngine{
  ...
  async runDetailed(location:string,custom?:string,objective?:Partial<DiscoveryObjective>):Promise<{results:DiscoveryResult[];executions:ProviderExecution[];strategies:SearchStrategy[];stopReason:string}>{
    const target:DiscoveryObjective={location,goal:'find companies with recurring transport needs compatible with a Renault Master',maxSearchRequests:24,maxNewCompanies:100,maxRuntimeSeconds:60,minMarginalNovelty:0.1,...objective};
