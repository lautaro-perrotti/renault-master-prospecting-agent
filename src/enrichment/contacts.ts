...
export type ContactFinding={email?:string;phone?:string;kind:'email'|'phone';role:ContactRole;sourceUrl:string;priority:number;verificationStatus:VerificationStatus};
...
export function extractContacts(text:string,sourceUrl:string){
  ...
  for(const email of emails){const local=email.split('@')[0];const p=priority.indexOf(local);findings.push({email,kind:'email',role:roleForEmail(email),sourceUrl,priority:p>=0?p:priority.length,verificationStatus:'SYNTAX_VALID'})}
  for(const phone of [...new Set((text.match(phonePattern)??[]).filter(validPhone).map(normalizePhone))])findings.push({phone,kind:'phone',role:'UNKNOWN',sourceUrl,priority:99,verificationStatus:'PUBLICLY_OBSERVED'});
