...
},t=>[uniqueIndex('contacts_company_email_uq').on(t.companyId,t.email),index('contacts_email_idx').on(t.email)]);
...
},t=>[uniqueIndex('raw_search_provider_query_url_uq').on(t.provider,t.query,t.resultUrl),index('raw_search_run_idx').on(t.runId)]);
